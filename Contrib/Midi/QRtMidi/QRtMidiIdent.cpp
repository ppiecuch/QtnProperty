...
//-------------------------------------------------------------------------
QString QRtMidiDevIdent::toString()
{
    QString str;
...
void QRtMidiDevIdent::fromIdentData( const QRtMidiData & data )
{
    ...
    if(data.contains("F0 7E XX 06 02") && len >= 13)
    {
        ...
        if(len >= 13+adj)
        {
            ...
            SOXHdr    = QRtMidiData("F0") + Manufacture + Family.mid(0,1) + Product.mid(0,1);
...
//-------------------------------------------------------------------------
quint16 QRtMidiDevIdent::getProductId()
{
    int rtval = 0;
    if(!Product.isEmpty())
    {
...
//-------------------------------------------------------------------------
quint8 QRtMidiDevIdent::getProductByte()
{
    return (quint8)getProductId()&0xFF;
}

//-------------------------------------------------------------------------
unsigned short QRtMidiDevIdent::getFamilyId()
{
    int rtval = 0;
    if(!Family.isEmpty())
    {
...
//-------------------------------------------------------------------------
quint8 QRtMidiDevIdent::getFamilyByte()
{
    return (quint8)getFamilyId()&0xFF;
}

//-------------------------------------------------------------------------
QString QRtMidiDevIdent::getShortId()
{
    QString rtval;
