...
//-------------------------------------------------------------------------
void QRtMidiData::copyToStdVec( std::vector<unsigned char>& vec, int offset /* = 0*/, int len /*= -1 */) const
{
    vec.clear();
...
//-------------------------------------------------------------------------
QList<QRtMidiData> QRtMidiData::split( int maxSz ) const
{
    QList<QRtMidiData> rtlst;
    if(0 == maxSz)
    {
        return rtlst;
    }
    ...
    if(len > maxSz)
    {
        ...
        for (int i = 0; i < chunkcnt ; i++)
        {
            ...
            rtlst.append(QRtMidiData(this->mid(offset,maxSz)));
            offset += maxSz;
        }

        if(remSize)
        {
            rtlst.append(QRtMidiData(this->mid(offset,remSize)));
        }
...
//-------------------------------------------------------------------------
std::vector<unsigned char> QRtMidiData::toStdVector(int offset /*=0*/, int len /*= -1*/)
{
...
void QRtMidiData::setData(const char* data, ...)
{
    clear();
...
//-------------------------------------------------------------------------
bool QRtMidiData::match( const char* val,bool must_start_with /* = false */) const
{
    QString tst = val;
...
//-------------------------------------------------------------------------
bool QRtMidiData::match( QRegExp rx,bool must_start_with /*= false*/) const
{
    QString thisData = this->toString();
    bool rtval;
    int pos = rx.indexIn(thisData);
...
//-------------------------------------------------------------------------
bool QRtMidiData::contains( const char* hex_string) const
{
...
bool QRtMidiData::startsWith(const char *hex_string) const
{
    return this->match(hex_string,true);
}

bool QRtMidiData::startsWith(const QByteArray& ba) const
{
    return this->match(ba,true);
}

bool QRtMidiData::startsWith(const QRtMidiData &md) const
{
    return this->match(md.toByteArray(),true);
}

//-------------------------------------------------------------------------
unsigned char QRtMidiData::sumOfSection( int start, int len) const
{
    int accum = 0;
...
//-------------------------------------------------------------------------
QString QRtMidiData::toAsciiString( int offset, int len )
{
    QString rtval;
