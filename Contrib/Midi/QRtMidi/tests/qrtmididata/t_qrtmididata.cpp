...
class t_QRtMidiData: public QObject
{
    Q_OBJECT
...
private slots:
    ...
    void set14param()
    {
        QRtMidiData md("F0 01 02 03 04 F7");
        int val = 0x81;
    ...
    {
        QByteArray ba = QRtMidiData::numHexToByteArray(0x41424344);
    ...
    void lengthTest()
    {
        QRtMidiData md("F0 01 02 F7");
        QCOMPARE(md.length(),4);
    }

    void testSplit()
    {
        QRtMidiData md("F0 70 71 72 73 74 75 76 F7");

        QList<QRtMidiData> lst = md.split(2);
...
private:
    ...
    void checkContainsHelper(QRtMidiData md,bool b)
    {
        qDebug() << md.toString();
...
QTEST_MAIN(t_QRtMidiData);

#include "t_qrtmididata.moc"
